...
export default function Books() {
  ...
  if (categoryId) formData.append('categoryId', categoryId);
      ...
      // If authorId points to an existing author but the name was changed in the book form,
      // propagate that rename to the Authors resource so the Authors page shows the update.
      try {
        if (resolvedAuthorId) {
          const existing = authors.find(a => String(a.id) === String(resolvedAuthorId));
          const newName = (authorName || '').trim();
          if (existing && newName && existing.name !== newName) {
            // Update author on backend and update local authors state optimistically
            try {
              await api.updateAuthor(resolvedAuthorId, { name: newName });
              setAuthors(prev => prev.map(a => a.id === existing.id ? { ...a, name: newName } : a));
              // Let other pages know authors/books may have changed
              window.dispatchEvent(new CustomEvent('booksUpdated', { detail: { action: 'author-renamed', author: { id: existing.id, name: newName } } }));
            } catch (e) {
              // Non-fatal: continue to update the book even if author rename fails
              console.error('Failed to update author name while saving book', e);
            }
          }
        }
      } catch (e) {
        console.error('Author update check failed', e);
      }
